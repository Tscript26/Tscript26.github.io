...
- 应用层：提供用户服务接口，直接支持应用程序的数据交换
- 表示层：转换数据格式，确保不同系统间兼容性
- 会话层：建立、维护、终止逻辑链接，管理数据交换的同步点
- 传输层：提供端到端的可靠传输服务，支持TCP/UDP协议，保证数据完整性与顺序；会话管理支持多数据同步和错误恢复
- 网络层：负责路由选择和分组转发，引入IP地址进行逻辑寻址，核心协议是IP，路由器在此层根据目的地址选择路径
- 数据链路层：将数据封装成帧，并处理错误检测、流量控制等，关键设备为交换机，通过MAC地址实现同一网段的通信
- 物理层：负责数据在物理介质上传输的原始比特流，定义了电气，光学等接口标准，关键设备为集线器，负责信号放大与转发
